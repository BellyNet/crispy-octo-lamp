  ...
  for (const url of urls) {
    try {
      ...
      const username = await page.evaluate(() => {
        ...
        return anchors.length ? anchors[anchors.length - 1].textContent.trim() : 'unknown_cow';
      });

      const { images: imageFolder, webm: webmFolder } = createModelFolders(username);

      await page.waitForSelector('#theMainImage, video source', { timeout: 10000 });
      ...
      const fatLabels = ['plump', 'luscious', 'waddler', 'milkme', 'snackload', 'stuffed', 'overflowing'];
      ...
      if (ext === '.gif') {
        ...
        if (isAnimated) {
          const mp4Path = path.join(webmFolder, `${username}_${label}-${padded}.mp4`);

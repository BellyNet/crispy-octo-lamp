...
const fatLabels = ['fat', 'luscious', 'waddler', 'milkme', 'snackload', 'stuffed', 'overflowing'];
const gifsToConvert = [];
let totalCount = 0;
...
(async () => {
  ...
  if (!startUrl) return console.error("⚠️  Usage: node milkmaid.js <gallery-url>");
...
await subPage.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0 Safari/537.36');
await subPage.setViewport({ width: 1280, height: 800 });
  ...
  const { images: imageFolder, webm: webmFolder } = createModelFolders(modelName);
...
try {
  ...
  console.warn('⚠️ No video play button or timed out waiting for video to load');
}
...
const mediaUrl = await subPage.evaluate(() => {
  ...
  return null;
    ...
    try {
      const ext = path.extname(new URL(mediaUrl).pathname).split('?')[0].toLowerCase();
      ...
      const finalPath = path.join(ext === '.gif' ? webmFolder : imageFolder, filename);
      ...
      if (knownHashes.has(hash)) {
        ...
        if (ext === '.gif' && buffer.includes(Buffer.from('NETSCAPE2.0'))) {
          const mp4Path = path.join(webmFolder, `${modelName}_${label}-${padded}.mp4`);
  ...
  console.log(`🚜 Starting gif-to-mp4 conversions: ${gifsToConvert.length} total`);
...
Errors:              ${errorCount}
  ...
  await browser.close();
  console.log(`🎉 Gallery complete: ${imageCounter - 1} images milked from ${modelName}`);
})();

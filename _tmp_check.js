...
function updateStats() {
  ...
  for (const [type, label] of Object.entries(labels)) {
    ...
    const smpBtn = document.querySelector(
      `.smp-filter-btn[data-type="${type}"]`
    )
...
function updateDateRange() {
  ...
  if (activeSort === 'added-asc' || activeSort === 'added-desc') {
    ...
    const first =
      withAdded[activeSort === 'added-asc' ? 0 : withAdded.length - 1]
    const last =
      withAdded[activeSort === 'added-asc' ? withAdded.length - 1 : 0]
  ...
  const earliest = withDates[activeSort === 'asc' ? 0 : withDates.length - 1]
  const latest = withDates[activeSort === 'asc' ? withDates.length - 1 : 0]
